...
- [ ] **Accès initial Client/Salarié par code provisoire (feat/client-employee-initial-access-code, une fois la PR mergée)** : complément à l'invitation par lien magique existante (`client-provision`/`employe-provision`, inchangés) — le patron peut désormais générer depuis `client-fiche.html`/`employe-fiche.html` un code à 8 caractères, envoyé par email ou révélé une seule fois à l'écran, que le client/salarié saisit lui-même sur `client-connexion.html`/`employe-connexion.html` (« Première connexion ») avant de créer son mot de passe. **3 actions manuelles avant que ça fonctionne en production** :
  1. Rejouer **`migrations/2026-08-03-provisional-access-code.sql`** sur la base Supabase réelle (idempotent) — active `pgcrypto`, crée la table `provisional_access_codes` et les RPC associées.
  2. Déployer **`supabase-functions/create-access-code.ts`** sous le nom `create-access-code` (`verify_jwt` = ON, comme `client-provision`/`employe-provision`).
  3. Déployer **`supabase-functions/activate-access-code.ts`** sous le nom `activate-access-code` (`verify_jwt` = **OFF** — fonction publique, appelée avant authentification, durcie côté code : CORS restreint, rate limiting IP, validation stricte).
  Utilise les mêmes secrets `RESEND_API_KEY`/`RESEND_FROM` que `send-email.ts` (section 2 ci-dessous) — si absents, l'envoi par email échoue proprement (`delivery_status: 'delivery_failed'`, jamais affiché comme un succès) mais l'option « Générer et afficher une fois » fonctionne sans aucune dépendance email.
...
1. ~~`employe-auth`~~ / ~~`employe-set-pin`~~ *(RETIRÉES 2026-07-19)* : l'ancien modèle PIN 4 chiffres badgé sur l'appareil patron est abandonné au profit de l'authentification universelle (voir point 7 ci-dessous). Si ces deux fonctions sont encore déployées sur ton projet Supabase, tu peux les supprimer (Edge Functions → ... → Delete) — elles ne sont plus appelées par aucune page.
...
5. *(numéro retiré — voir point 1 ci-dessus)*
6. **`client-provision`** *(ajouté 2026-07-19, Espace Client — passé en mode invitation le même jour, authentification universelle)* : **invite** le client par email (`auth.admin.inviteUserByEmail`, il choisit lui-même son mot de passe) depuis `clients.html`/`client-fiche.html` dès qu'un email est renseigné — **jamais** un `signUp()`/`inviteUserByEmail()` côté navigateur du patron (ça le déconnecterait de son propre compte). Déploie **`supabase-functions/client-provision.ts`** sous le nom `client-provision`. Aucune nouvelle clé.
7. **`employe-provision`** *(ajouté 2026-07-19, authentification universelle — remplace `employe-auth`/`employe-set-pin`)* : miroir exact de `client-provision` côté employé — invite l'employé par email depuis `equipe.html`/`employe-fiche.html` dès qu'un email est renseigné. Déploie **`supabase-functions/employe-provision.ts`** sous le nom `employe-provision`. Aucune nouvelle clé.
...
**Authentification universelle Client + Employé** *(Client ajouté 2026-07-19, étendu à l'employé le même jour — remplace le modèle PIN/badge-sur-appareil-patron)* : tables `client_accounts`/`client_requests`/`employe_accounts` + fonctions RPC `get_my_client_profile`/`get_my_employee_profile`/`get_my_employee_interventions`, plus une **réécriture des policies `seba_messages`** (un client ou un employé authentifié a désormais son propre `auth.uid()`, distinct de celui du patron — l'ancienne policy `auth.uid() = user_id` seule ne suffit plus). Si le schéma complet a déjà été exécuté avant cette date, rejoue **`migrations/20260719_client_espace.sql`** PUIS **`migrations/20260719c_universal_auth_employe.sql`** (dans cet ordre) — idempotents, sûrs à rejouer même si une partie existe déjà. Nécessite aussi le déploiement des Edge Functions `client-provision` et `employe-provision` (points 6-7 ci-dessus) — sans elles, un client/employé ne peut jamais obtenir d'accès.
