...
async function newRolePage(browser) {
  ...
  page.on('console', async (m) => {
    ...
    // muet ici.
    if (m.type() !== 'error' || /manifest\.json|404 \(Not Found\)|502 \(Bad Gateway\)|403 \(Forbidden\)/.test(m.text())) return;
...
async function main() {
  ...
  try {
    ...
    await client.page.goto(`http://localhost:${PORT}/client-connexion.html`, { waitUntil: 'domcontentloaded' });
    ...
    console.log('\n== [10/20] Retour vers le document demandé après une connexion fraîchement activée ==');
    await client.page.evaluate(() => SebaDB.clientPortal.logout());
    const deepLink = `http://localhost:${PORT}/client-espace.html?doc=quote&id=dev_qaaa1`;
    await client.page.goto(deepLink, { waitUntil: 'domcontentloaded' });
    await new Promise((r) => setTimeout(r, 500));
    assert(client.page.url().includes('client-connexion.html') && client.page.url().includes('return='), 'session absente -> redirection avec return= conservé');
    await client.page.type('#identifiant', clientEmail);
    await client.page.type('#password', CLIENT_NEW_PW2);
    await client.page.click('#btn-connect');
    await new Promise((r) => setTimeout(r, 1200));
    assert(client.page.url().includes('devis-document.html?id=dev_qaaa1'), 'retour réel au document demandé après connexion (' + client.page.url() + ')');
    ...
    console.log('\n== [13/20] Connexion employé -- clic réel ==');
    await employe.page.goto(`http://localhost:${PORT}/employe-connexion.html`, { waitUntil: 'domcontentloaded' });
    ...
    await patron.page.goto(`http://localhost:${PORT}/connexion.html`, { waitUntil: 'domcontentloaded' });
    const patronMobile = await patron.page.evaluate(() => ({
    ...
    assert(patronMobile.btnVisible && !patronMobile.hScroll, 'connexion.html utilisable à 390px, aucun scroll horizontal');
    await client.page.setViewport({ width: 390, height: 844 });
    await client.page.goto(`http://localhost:${PORT}/client-connexion.html`, { waitUntil: 'domcontentloaded' });
    const clientMobile = await client.page.evaluate(() => ({
      btnVisible: !!document.getElementById('btn-connect').offsetParent,
      hScroll: document.documentElement.scrollWidth > document.documentElement.clientWidth + 2,
    }));
    assert(clientMobile.btnVisible && !clientMobile.hScroll, 'client-connexion.html utilisable à 390px, aucun scroll horizontal');
    await employe.page.setViewport({ width: 390, height: 844 });
    await employe.page.goto(`http://localhost:${PORT}/employe-connexion.html`, { waitUntil: 'domcontentloaded' });
    const empMobile = await employe.page.evaluate(() => ({
      btnVisible: !!document.getElementById('btn-connect').offsetParent,
      hScroll: document.documentElement.scrollWidth > document.documentElement.clientWidth + 2,
    }));
    assert(empMobile.btnVisible && !empMobile.hScroll, 'employe-connexion.html utilisable à 390px, aucun scroll horizontal');
    await patron.page.setViewport({ width: 1280, height: 900 });
    await client.page.setViewport({ width: 1280, height: 900 });
    await employe.page.setViewport({ width: 1280, height: 900 });
