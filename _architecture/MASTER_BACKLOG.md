...
| CED-009 | P2 | Customer Email Delivery | Aucune Draft PR n'existe pour `feature/customer-email-delivery` | EN COURS | Aucune | Ouvrir une Draft PR, y coller la mention de blocage déploiement (déjà rédigée dans `DEPLOYMENT_CHECKLIST.md`) | `gh pr list --state all` : aucune PR pour cette branche |
