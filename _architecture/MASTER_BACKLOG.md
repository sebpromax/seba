...
| DASH-EMPTY-001 | Un compte patron neuf n'affiche plus aucune donnée fictive. **Bug plus grave que prévu, trouvé en validant** : `SebaDB.ready()` (`docs/seba-data.js`) générait 6 faux clients + devis/factures/interventions/employés/journal dès la première visite et les **poussait réellement vers Supabase** (pas seulement un affichage) pour TOUT nouveau patron connecté — corrigé en gatant `seed()` sur `!hasSupabase` (jamais pour un compte réellement connecté). Complément : `DASHBOARD_DEMO_DATA` (fausses missions du jour) supprimé de `docs/app/dashboard.html`, remplacé par un bloc "Bienvenue dans Seba" + états vides réels déjà existants, interface complète conservée. Validé contre un vrai compte Supabase local (pas seulement mode démo) : 0 client fictif poussé, 0 faux nom affiché, bloc de bienvenue puis vraie création de client sans mélange, 0 erreur console | PR #97 (`fix(dashboard): show real empty states for new accounts`, Ready, mergée), commit `0ae6906`/merge `606ed9c` |
