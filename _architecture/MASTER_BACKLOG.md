...
|---|---|---|---|---|---|---|---|
| DASH-EMPTY-001 | P1 | Onboarding / Dashboard | Un compte patron neuf affiche des données fictives (chiffre d'affaires, activité, clients/devis/factures/interventions de démonstration) — le patron ne doit jamais se demander pourquoi son compte neuf a déjà du chiffre d'affaires | EN COURS | Bloque PILOT-004 (le pilote doit démarrer sur un dashboard honnête) | Identifier la source des données affichées (dashboard patron, métriques, activité récente, fallbacks JS), remplacer par des états vides réels, garder l'interface complète | Bénéficiaire : nouveau patron. Résultat observable : un compte patron neuf affiche une interface complète mais uniquement des états vides fondés sur ses vraies données |
| PILOT-004 | P0 | Pilote produit | Premier cycle métier réel en production : connexion patron → configuration entreprise → création client → devis → consultation/acceptation client → intervention → planning → assignation → exécution mobile → compte rendu/preuve → approbation client → validation patron → facture → paiement → historique client complet (15 étapes) | À FAIRE | DASH-EMPTY-001 (reprend immédiatement après cette correction) | Exécuter le cycle sur la version réellement déployée, avec un utilisateur réel (pas un test automatisé seul) ; consigner chaque étape dans `_architecture/PILOT_RUN_001.md` | `_architecture/PILOT_RUN_001.md` |

Seule tâche produit `EN COURS` (doctrine "Limite des chantiers", `CLAUDE.md`) : DASH-EMPTY-001. PILOT-004 reprend dès sa livraison. Aucune autre tâche active (voir "À faire ensuite" pour les tâches P2 en attente : CORE-001, CORE-004, PILOT-002).
