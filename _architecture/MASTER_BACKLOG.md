...
| AUTH-004 | P1 | Authentification | `fix/invitation-delivery` (retry + statut de livraison des invitations) n'est ni mergée ni en PR, et modifie les mêmes fichiers que `feature/customer-email-delivery` (`client-provision.ts`, `employe-provision.ts`) | EN COURS | Aucune | Décider : merger dans `main` avant de continuer CED, ou rebaser `feature/customer-email-delivery` dessus — sinon risque de conflit/régression silencieuse | Branche `fix/invitation-delivery` (commit `7193874`, 2026-07-23), non présente dans `git branch --merged main` |
...
| CED-009 | P2 | Customer Email Delivery | Aucune Draft PR n'existe pour `feature/customer-email-delivery` | À FAIRE | Aucune | Ouvrir une Draft PR, y coller la mention de blocage déploiement (déjà rédigée dans `DEPLOYMENT_CHECKLIST.md`) | `gh pr list --state all` : aucune PR pour cette branche |
