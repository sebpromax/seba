...
| Portail client — provisioning/invitation | `client-provision` (fonction Edge, déclenchée depuis `clients.html`) | Supabase Auth (invite email) | — | Oui (mais échoue) | Oui | **IMPLEMENTED_BROKEN — P0** | `500 {"error":"Erreur serveur"}` systématique à chaque création de client, même root cause probable que la panne de signup |
