...
| Facture — correction d'un paiement erroné | `factures.html` → "Enregistrer un paiement" | — | — | Oui | Oui | **NOT_IMPLEMENTED — P1 PILOT GATE** | Diagnostic corrigé après relecture du code — voir `QA360_P0_REMEDIATION_PLAN.md` section 4 (l'ancien diagnostic "succès fantôme" était un faux positif : `recordPayment()`, `docs/seba-data.js:4318`, rejette déjà les montants ≤ 0 localement). Ce qui reste confirmé : aucun mécanisme UI de correction/annulation de paiement n'existe (montants négatifs rejetés, pas de bouton de suppression) — absence de fonctionnalité, classée **P1 — PILOT GATE** (ne bloque pas cette passe, doit être résolue ou explicitement acceptée avant l'ouverture du pilote). Facture réelle #F-0095 (Marc Roussel) porte toujours un paiement de test de 40€, **en attente de validation explicite du fondateur avant tout nettoyage** — procédure complète dans `QA360_P0_REMEDIATION_PLAN.md` section 2a (id paiement : `id_ms7crz6db6iyj`) |
