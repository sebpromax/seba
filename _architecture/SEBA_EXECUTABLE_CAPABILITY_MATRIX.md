...
| Portail client — provisioning/invitation | `client-provision` (fonction Edge, déclenchée depuis `clients.html`) | Supabase Auth (invite email) | — | Oui (mais échoue) | Oui | **IMPLEMENTED_BROKEN — P0** | `500 {"error":"Erreur serveur"}` systématique à chaque création de client — même symptôme générique que la panne de signup, cause commune non confirmée (hypothèse à vérifier par les logs, voir `QA360_P0_REMEDIATION_PLAN.md`) |
...
| Facture — correction d'un paiement erroné | `factures.html` → "Enregistrer un paiement" avec montant négatif | `sync-push` | **Non confirmée** | Oui | Oui | **IMPLEMENTED_BROKEN** | Un paiement de -40€ soumis en correction a fait passer l'AFFICHAGE à "En attente", mais la requête directe au serveur montre que **seul le paiement +40€ original existe** — la correction n'a jamais été persistée côté serveur alors que l'interface a affiché un succès. Facture réelle #F-0095 (Marc Roussel) laissée avec un paiement de test de 40€ non réversible par l'UI — **nettoyage manuel nécessaire côté fondateur** (id paiement : `id_ms7crz6db6iyj`) |
...
## Point d'architecture à signaler (pas un bug, une observation structurelle)

Le schéma (`supabase-schema.sql`) contient à la fois :
- une table `seba_state` (JSONB unique par compte, `select=state&account=eq...`) — **c'est la source de vérité réellement lue et écrite par toutes les pages patron testées** (clients, planning, devis, factures) ;
- des tables normalisées classiques : `clients`, `interventions`, `devis`, `factures`, `paiements`, `employes`, etc.

Le mécanisme d'écriture n'est pas un écrasement brutal du blob : `sync-push` applique des patchs versionnés (`client_seq`, `version`, `last_snapshot`) — un vrai système de suivi de version existe, ce n'est pas du "dernier écrivain gagne" naïf. Mais la relation exacte entre `seba_state` (lu par le patron) et les tables normalisées (probablement utilisées par les portails client/employé et les RLS) **n'a pas été vérifiée dans cette passe** — c'est un point à clarifier avant de considérer l'architecture de données comme stable, car une divergence entre les deux systèmes est le genre de risque qui ne se voit qu'en le cherchant spécifiquement.
...
- Paiement de 40€ sur la facture réelle **#F-0095** (Marc Roussel, id paiement `id_ms7crz6db6iyj`) — la tentative de correction via un montant négatif a échoué silencieusement côté serveur (voir ligne "Facture — correction d'un paiement erroné" ci-dessus). Nécessite une intervention manuelle (Supabase dashboard/SQL) pour retirer proprement ce paiement de `seba_state`.
