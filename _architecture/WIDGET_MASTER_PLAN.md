...
| 5 | 2026-07-15 | Dette de synchronisation corrigée ("25 widgets" → 26, §3/§5). Nouvelle section §19bis : alignement avec une proposition d'architecture élargie externe — écartée en l'état (TypeScript/build step incompatible avec CLAUDE.md), idées de fond reprises en JS vanilla : `SEBA_DASHBOARD_CONFIG.explainCompatibility()` (additif, `docs/services/config-dashboard.js`), taxonomie des widgets appliquée à l'inventaire réel, gaps documentés honnêtement (modèle de données étendu absent de SebaDB, scoring de priorité/dashboards par rôle/event bus non implémentés — dépendances manquantes réelles, pas juste non prioritaires). | Aucun changement de comportement visible (fonction additive uniquement) | Non |
...
## 19bis. Alignement avec une proposition d'architecture élargie (2026-07-15)

Une proposition externe (document "SEBA — Widget & Data Master Plan", 19 sections) a suggéré un plan de données universel, un moteur de compatibilité explicable, un scoring de priorité pondéré, des dashboards par rôle, une architecture événementielle et une roadmap de 32 secteurs — sous forme de boilerplate **TypeScript avec build step** (`WidgetRegistry`/`SectorRegistry` en classes, tests Vitest).

**Écarté tel quel** : incompatible avec la contrainte fondamentale du projet (`CLAUDE.md` — "HTML5/CSS3/JS vanilla, zéro bundler, zéro framework"). TypeScript nécessite une compilation que GitHub Pages ne fait pas ; adopter ce boilerplate aurait créé une seconde architecture parallèle, jamais servie, déconnectée de `docs/widgets.js`/`docs/services/config-dashboard.js` réellement en production. Les idées de fond restent valables et sont reprises ci-dessous en JS vanilla, sans build step.

### Implémenté ce jour

- **Compatibilité explicable** (inspiré du `CompatibilityResult` de la proposition) : `SEBA_DASHBOARD_CONFIG.explainCompatibility(widgetId, secteur)` (`docs/services/config-dashboard.js`), additif — `isCompatible()` reste inchangée pour `buildLibraryPanelHTML()`. Renvoie `{ compatible, status: 'O'|'P'|'X', reasons: string[] }`, y compris la traçabilité d'un repli `WIDGET_SECTOR_FALLBACK` (WM-002) dans les raisons. Aucun consommateur aujourd'hui (UI "pourquoi ce widget n'est pas proposé ?" non construite) — utilitaire prêt, pas encore branché à une UI.
- **Flag `availability: universal/restricted`** (proposition §13) : **non ajouté** — vérifié que l'ambiguïté qu'il corrige (un tableau `supportedSectors: []` vide pouvant signifier "aucun secteur" ou "tous") **n'existe pas dans notre système** : la compatibilité n'est jamais déclarée par widget individuel mais calculée depuis des listes centralisées (`CORE`, `BY_SECTEUR.commun`, `BY_SECTEUR[secteur]`) — un widget n'a pas de champ `supportedSectors` à interpréter. Le problème que ce flag résout est déjà résolu autrement chez nous.

### Repris comme vocabulaire de classification (sans changement de code)

**Taxonomie des widgets** (proposition §5) appliquée à l'inventaire réel (26 widgets, `docs/widgets.js`) :

| Catégorie | Widgets concernés |
|---|---|
| Informatif | `metric-0..3`, `bento-chart` (avant analyse), `workspace`, `portal` |
| Analytique | `chart-donut`, `lot-treso`, `marge-reelle` |
| Alerte | `lot-impayes` |
| Prédictif | Aucun aujourd'hui — `lot-tournee`/`lot-carte` détectent des conflits géographiques déjà survenus, pas un risque futur |
| Prescriptif | `recos` |
| Opérationnel | `bento-actions`, `quick-actions`, `serenity-score` (ouvre le mode focus), `generic-media-report`/`marge-reelle` (lien vers l'action) |

Cette classification n'est pas stockée dans `WIDGET_CATALOG` (aucune structure modifiée) — c'est une grille de lecture documentaire, à ajouter comme champ réel seulement si un futur besoin (filtrage par type dans une UI) le justifie.

### Vision/roadmap (non implémenté, dépendances manquantes documentées honnêtement)

- **Modèle de données universel étendu** (contrats, prospects, produits/stocks, équipements, véhicules, incidents) : **absent de SebaDB aujourd'hui** — `docs/seba-data.js` ne couvre que `clients, devis, factures, interventions, employes, journal`. Chaque nouvelle entité listée dans la proposition est un chantier de schéma SebaDB à part entière, pas une extension du moteur de widgets.
- **Scoring de priorité pondéré** (formule §9 de la proposition) : **non implémenté** — construire ce moteur aujourd'hui obligerait à inventer des signaux (`urgency`, `confidence`, `businessImpact` par widget) qui n'existent dans aucune donnée réelle ; violerait la règle "jamais de donnée inventée" tenue tout au long de ce document. `defaultOrder` (statique, `WIDGET_CATALOG`) reste le seul mécanisme d'ordre aujourd'hui.
- **Dashboards par rôle** : **bloqué en amont** par R-09 (aucun système de permissions/rôles n'existe dans Seba — le champ `role` d'un employé est un intitulé de poste, pas un rôle d'accès). Prérequis à construire avant tout dashboard différencié par rôle.
- **Architecture événementielle** (`InterventionCompleted`, `InvoiceCreated`...) : `docs/src/ui/event-bridge.js` existe déjà comme fondation partielle (Phase 3 "non confirmée" selon ses propres commentaires) — le moteur de widgets actuel reste à rafraîchissement manuel (`renderGrid()` appelé explicitement), pas événementiel. Alignement naturel pour une future itération, non fait ici.
- **Automatisations sectorielles** : aucune aujourd'hui, chantier séparé.

### Roadmap de verticalisation — reconciliation avec l'existant

La proposition suggère 10 secteurs prioritaires (Phase 3) : nettoyage, conciergerie, maintenance immobilière, plomberie, climatisation, jardinage, lutte antiparasitaire, automobile, nautisme, transport.

**Déjà couverts par les 11 secteurs réels** (`businessTypes.js`) : nettoyage → `menage`, conciergerie → `conciergerie`/`conciergerieCopro`/`conciergerieEntreprise`, jardinage → `jardinage`. **Plomberie et climatisation ne sont pas des secteurs distincts aujourd'hui** — ce sont des *services* à l'intérieur du secteur générique `maintenance` (`businessTypes.js` l.263-307, confirmé §4.3 de ce document). **Automobile, nautisme, transport, lutte antiparasitaire, maintenance immobilière (au sens tertiaire/copro)** : aucune couverture actuelle, cohérent avec le brainstorm de niches à fort potentiel déjà produit dans cette conversation (entretien piscine, climatisation, nettoyage pro bureaux notamment déjà signalés indépendamment). **Décision produit non tranchée ici** : élever plomberie/climatisation au rang de secteurs à part entière (avec leur propre `BY_SECTEUR`) reste un choix produit, pas une correction technique — à ajouter comme nouvelle décision ouverte si jugé prioritaire.
