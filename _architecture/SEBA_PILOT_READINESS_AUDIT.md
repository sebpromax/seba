...
### P0-4 — RETIRÉ, remplacé par un constat plus précis (voir `QA360_P0_REMEDIATION_PLAN.md` section 4)
⚠️ **Correction post-audit** : cette entrée affirmait initialement que l'interface affichait un faux succès après une tentative de correction de paiement par montant négatif. En relisant le code (`recordPayment()`, `docs/seba-data.js:4318`), ce diagnostic était incorrect ou incomplet — la fonction **rejette déjà** tout montant ≤ 0 localement, avant tout envoi au serveur. Le texte "En attente" observé n'a pas de cause confirmée ; le plus probable est un rafraîchissement d'affichage sans lien avec cette tentative précise, pas une preuve de succès fantôme. Retiré de la liste des P0 confirmés. Ce qui reste réellement confirmé (voir le document de remédiation pour le détail) : (1) aucun mécanisme de correction/annulation de paiement n'existe dans l'UI — un vrai manque fonctionnel, pas ce bug-ci ; (2) le modèle d'écriture local-d'abord/synchro-arrière-plan est le modèle général de toute l'application (pas spécifique aux paiements), avec un indicateur visible de synchronisation en attente/échouée, mais sans confirmation immédiate avant d'afficher un succès — un choix d'architecture existant, pas un bug caché.
...
- **Aucun mécanisme de correction de paiement fiable** : il n'existe visiblement aucun moyen dans l'UI de retirer ou corriger proprement un paiement mal saisi (pas de bouton "annuler ce paiement" sur la fiche facture ; les montants négatifs sont rejetés par validation, sans alternative proposée).
