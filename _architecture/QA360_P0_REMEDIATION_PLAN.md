...
## 1. Cause des erreurs email — historique complet, deux défauts successifs

**Ceci ne remplace pas le diagnostic précédent, il le complète.** Deux pannes distinctes et successives ont été observées sur le chemin `send-email` (méthode : script isolé `scratchpad/qa/test-send-email-resend.mjs`, non commité, identifiants saisis localement par le fondateur, jamais transmis ni journalisés — appel direct de la fonction Edge déployée `send-email` avec un compte patron réel) :

**Défaut 1 (antérieur, désormais corrigé)** : la `RESEND_API_KEY` alors configurée était invalide — Resend répondait `HTTP 401 "API key is invalid"`. Panne réelle, rapportée par le fondateur, corrigée par le remplacement de la clé avant le test décisif ci-dessous. Je n'ai pas observé ce premier échec directement (rapporté par le fondateur, pas reproduit par moi) — consigné pour l'historique, pas comme une preuve de première main.

**Défaut 2 (actuel, confirmé par test direct, non résolu)** : après remplacement de la clé, `POST /functions/v1/send-email` → `200`, l'API Resend accepte la requête et crée un message avec un identifiant. Mais le statut de livraison réel du message est **`failed`**, événement exact : *"Domain is not verified: The domain used to send this email needs to be verified."*

**Cause actuelle de non-livraison** : l'identité expéditrice `onboarding@resend.dev` (domaine partagé par défaut de Resend, jamais vérifié) — pas la clé API, qui fonctionne désormais. La requête HTTP réussit (`200`) avant même que Resend tente la livraison réelle : un succès HTTP ne garantit donc rien ici, seul le statut de livraison compte. **`send-email` n'est donc toujours pas résolu** malgré le `200`.

**Lien de code établi, mais portée limitée — ne pas fermer les 3 P0 sur cette seule base** :
- `send-email.ts` appelle directement `https://api.resend.com/emails` avec sa propre `RESEND_API_KEY` — chemin testé directement, défaut 2 confirmé sur CE chemin précis.
...
**Ce lien de code est réel mais ne constitue PAS une preuve que ces 3 chemins sont résolus.** `RESEND_FROM` obligatoire (section 3) ne corrige que `send-email.ts`/`daily-digest.ts` — les deux appels directs à l'API Resend. Il **ne corrige rien** dans le relais SMTP interne de Supabase Auth, qui a sa propre configuration (dashboard Supabase, pas ce dépôt) et pourrait avoir une adresse expéditrice différente, une clé différente, ou un état différent de celui de `send-email`. **Aucun des 3 P0 (signup, client-provision, employe-provision) ne doit être considéré comme résolu avant réception ET ouverture réelle d'un email pour chacun des 3 chemins** (section 6/7 — tests requis, distincts les uns des autres).
...
⚠️ **EN ATTENTE DE VALIDATION EXPLICITE DU FONDATEUR — rien ci-dessous n'a été exécuté.** Le paiement de test `id_ms7crz6db6iyj` (40 €) est toujours présent sur cette facture réelle. Ne pas exécuter cette transaction avant accord explicite, même après lecture de ce document.
...
**P0-4 retiré en tant que bug confirmé.**
...
Détail de cette correction — `SebaDB.factures.recordPayment()` (`docs/seba-data.js:4318`) **valide et rejette déjà** tout montant ≤ 0, localement, avant tout appel serveur : `if (!amount || amount <= 0) return { ok: false, error: 'Montant de paiement invalide.' };`. Un montant de -40 est donc rejeté à ce stade, pas silencieusement accepté. Le texte "En attente" observé dans la liste après la tentative n'a pas de cause confirmée avec certitude — l'hypothèse la plus probable est un rafraîchissement d'affichage non lié à cette tentative précise (même famille que l'anomalie déjà notée sur le menu déroulant d'assignation employé), pas une preuve que la correction a été acceptée puis perdue.

**Conservé — reclassifié P1 (pas P0) :**
