...
- **Éradiquer une couleur = grep INSENSIBLE à la casse, sur toute la famille de nuances, mockups inclus.** Le pivot anti-bleu (PR #58) a raté 9 littéraux `#1d4ed8`/`#3b82f6`/`#dbeafe`/`#60a5fa` en minuscules dans des mockups d'illustration : les inventaires regex .NET sont sensibles à la casse par défaut, et `check-design-system` (mode diff) ne voit jamais les lignes non modifiées. Toujours balayer avec `IgnoreCase`, inclure les nuances voisines (blue-100→900 + `rgba(r,g,b,…)`), et re-vérifier le rendu LIVE après merge — c'est la vérification post-déploiement qui a débusqué le résidu. (Trouvé 2026-07-12, PR #59.)
